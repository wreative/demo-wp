window.uiAnimated_Mist = (el, canvas, userSettings = {}) => {
  const {
    Renderer,
    Program,
    Mesh,
  ...
  const fragment = `
        ...
        void main() {
            ...
            float t = uSpeed > 0.0 ? uTime * uSpeed : (uMouse.x + uMouse.y) * 1.5 + uProgress * 0.3;
            vec2 parallax = (uMouse - 0.5) * 0.1;
            vec2 uv = (fragCoord - 0.5 * uResolution.xy) / uResolution.y;
            uv *= uScale;
            uv.y *= -1.0;
            uv.y -= 0.4;
            uv += uIntensity * vec2(sin(uv.y * 6.283 + t * 0.5), cos(uv.x * 6.283 + t * 0.5)) * 0.05;
            uv += parallax;
            uv.xy += pixelOffset * 0.001;
            ...
            fragColor = cppn_fn(uv, 0.1 * sin(0.3 * t), 0.1 * sin(0.69 * t), 0.1 * sin(0.44 * t));


            vec4 col = fragColor;
            col.rgb = mix(uBackgroundColor, uColorStops[0] * 1.6, col.r);
            col.rgb = clamp(col.rgb, 0.0, 1.0);

            float grain = (random(gl_FragCoord.xy + t * 0.1) - 0.5) * 0.1;
            col.rgb += grain * uNoise;
  ...
  const normalizeSettings = raw => {
    ...
    if (raw.intensity !== undefined) {
      normalized.intensity = parseFloat(raw.intensity) * 0.05;
    }
