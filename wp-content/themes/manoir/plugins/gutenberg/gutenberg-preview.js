...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			manoir_create_observer( 'manoir-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				manoir_gutenberg_editor_init();
			} );
		} else {
		 	manoir_gutenberg_editor_init();
		}
	...
	function manoir_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'manoir_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'manoir_inited' ) ) {
	...
	// Init on page load
	function manoir_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = manoir_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + MANOIR_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + MANOIR_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in MANOIR_STORAGE['override_classes'] ) {
				$obj.addClass( MANOIR_STORAGE['override_classes'][i].replace( '%s', MANOIR_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( manoir_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && MANOIR_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'manoir_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			manoir_remove_observer( 'manoir-check-editor-styles-wrapper' );
			manoir_create_observer( 'manoir-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'manoir_inited' );
	}
} );
