...
jQuery( document ).ready(
	function() {
		...
		manoir_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="manoir_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.manoir_post_editor' );
						if ( link.length > 0 ) {
							manoir_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.manoir_post_editor', function(e) {
						manoir_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('manoir_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function manoir_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
